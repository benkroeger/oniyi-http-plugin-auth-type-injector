...
const formatPath = (path, requestOptions, typeToNameMap) => path.replace(formatPathRegex, (...args) => {
  ...
  // we want to allow falsy values only for 'authType' parameter
  // if template requirements are not met, {{templateString}} should be replaced in path
  if (name === 'authType' && isFalsy(mappedName)) {
    return '';
  }
 ...
 *
 * @param {String} text   Text that needs formatting
 * @return {*}
...
const formatString = (text) => {
  ...
  return text.replace(removeDoubleSlashesRegex, '$1');
};
