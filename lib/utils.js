 ...
 * Using 'typeToNameMap' to look for custom 'authType' mapping as a result from previous 'requestOptions' mapping.
 ...
 *
 * 'templateString' is left in the path by default for intentionally for debugging purposes
 *
...
const extractUriPath = (uri, callback) => {
  ...
  if (_.isString(uri)) {
    if (/:?\/\//.test(uri)) {
      return callback(null, decodeURIComponent(uri));
    }
  ...
  if (_.isObject(uri) && Object.getPrototypeOf(uri).constructor.name === 'Url') {
    ...
    return callback(null, decodeURIComponent(formattedUri));
  }
